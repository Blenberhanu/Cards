...
export class GridLayout extends React.Component<IGridLayoutProps, IGridLayoutState> {
  ...
  private _getItemCountForPage = (itemIndex: number, surfaceRect: IRectangle): number => {
    if (itemIndex === 0) {
      this._isCompact = surfaceRect.width < 480;
      if (this._isCompact) {
        this._columnCount = 1;
        this._columnWidth = surfaceRect.width;
      } else {
        this._columnCount = Math.ceil(surfaceRect.width / (4));
        this._columnWidth = Math.max(300, Math.floor(surfaceRect.width / this._columnCount) + Math.floor(20 / this._columnCount));
        this._rowHeight = this._columnWidth;
      }
    }

    return this._columnCount * 4;
  }

  private _getPageHeight = (): number => {
    return this._rowHeight * 4;
  }

  private _onRenderCell = (item: any, index: number | undefined): JSX.Element => {
    const isCompact: boolean = this._isCompact;
    const cellPadding: number = index % this._columnCount !== this._columnCount - 1 && !isCompact ? 20 : 0;
    const finalSize: ISize = { width: this._columnWidth, height: this._rowHeight };
    const cellWidth: number = isCompact ? this._columnWidth + 20 : this._columnWidth - 20;
    return (
      <div
        style={{
          width: `${cellWidth}px`,
